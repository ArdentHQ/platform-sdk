...
describe("WalletGate", ({ beforeAll, beforeEach, loader, nock, assert, it }) => {
	...
	beforeEach(async (context) => {
		...
		context.subject = new WalletGate(wallet);
	});

	it("#allows", (context) => {
		assert.false(context.subject.allows("some-feature"));
	});

	it("#denies", (context) => {
		assert.true(context.subject.denies("some-feature"));
	});

	it("#any", (context) => {
		assert.false(context.subject.any(["some-feature"]));
		assert.true(context.subject.any(["Client.transactions"]));
	});

	it("#all", (context) => {
		assert.false(context.subject.all(["some-feature"]));
		assert.true(context.subject.all(["Client.transactions"]));
	});
});
