...
describe("TransactionIndex", ({ beforeAll, beforeEach, nock, assert, it, loader }) => {
	...
	it("all", async (context) => {
		assert.instance(await context.subject.transactionIndex().all(), ExtendedConfirmedTransactionDataCollection);
	});

	it("sent", async (context) => {
		assert.instance(await context.subject.transactionIndex().sent(), ExtendedConfirmedTransactionDataCollection);
	});
	...
	it("should fetch transaction by id", async (context) => {
		...
		const transaction = await context.subject.transactionIndex().findById(transactionId);
	...
	it("should fetch transactions by id", async (context) => {
		...
		const fetchedIds = new Set(transactions.map((transaction) => transaction.id()));
		assert.true(fetchedIds.has(transactionId));
		assert.true(fetchedIds.has(secondaryTransactionId));
	});
});
