...
describe("ARK", ({ beforeAll, beforeEach, skip, it, nock, stub, assert, loader }) => {
	...
	beforeEach(async (context) => {
		nock.fake("https://ark-test.ardenthq.com:443")
			.get("/api/blockchain")
		...
		nock.fake("https://lsk-test.ardenthq.comcom:443")
			.get("/api/v2/accounts")
	...
	it("should sync", async (context) => {
		...
		nock.fake("https://ark-test.ardenthq.comcom:443")
			.get("/transactions")
			.query(true)
			.reply(200, [musig])
			.persist();
	...
	it("should add signature", async (context) => {
		nock.fake("https://ark-test-musig.ardenthq.comcom:443")
			.post("/", {
