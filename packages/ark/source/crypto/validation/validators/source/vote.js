'use strict';
import ucs2length from 'ajv/lib/compile/ucs2length.js'
import equal from 'ajv/lib/compile/equal.js';

var validate = (function() {
  var pattern0 = new RegExp('^[a-zA-Z0-9]+$');
  var pattern1 = new RegExp('^[+|-][a-zA-Z0-9]{66}$');
  var refVal = [];
  ...
  return function validate(data, dataPath, parentData, parentDataProperty, rootData) {
    ...
    if ((data && typeof data === "object" && !Array.isArray(data))) {
        ...
        if ((data1 && typeof data1 === "object" && !Array.isArray(data1))) {
          ...
          if (data2 === undefined) {
            ...
            var err = {
              ...
              params: {
                missingProperty: 'votes'
              },
              message: 'should have required property \'votes\''
            };
            if (vErrors === null) vErrors = [err];
            else vErrors.push(err);
            errors++;
              ...
              for (var i2 = 0; i2 < data2.length; i2++) {
                ...
                if (typeof data3 === "string") {
                  if (!pattern1.test(data3)) {
                    var err = {
                      keyword: 'pattern',
                      dataPath: (dataPath || '') + '.asset.votes[' + i2 + ']',
                      schemaPath: 'walletVote/allOf/0/pattern',
                      params: {
                        pattern: '^[+|-][a-zA-Z0-9]{66}$'
                      },
                      message: 'should match pattern "^[+|-][a-zA-Z0-9]{66}$"'
                    };
                    if (vErrors === null) vErrors = [err];
                    else vErrors.push(err);
                    errors++;
      ...
      if (data.recipientId !== undefined) {
        var errs_1 = errors;
        if (!refVal6(data.recipientId, (dataPath || '') + '.recipientId', data, 'recipientId', rootData)) {
